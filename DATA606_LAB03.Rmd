...
```
The mean of the men’s height looks greater than the mean of female heights. Men’s height distribution looks unimodal and symmetric. The distribution of female hieghts is unimodal and it seems to be skewed left.
...
qqline(sim_norm)
```
The outcome is similar to the previous normal probability plot because it strays towards the tails.
...
qqnormsim(fdims$wgt)
```
The distribution for weight does not seem as normal however. It is a bit of right skewness towards the tails.
...
pnorm(q = 170, mean = fhgtmean, sd = fhgtsd) # Theoretical probability
sum(fdims$hgt > 170) / length(fdims$hgt) # Empirical probability
```
```{r}
#Find the probability that a female weighs more than 75 KG?
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)
pnorm(q = 75, mean = fwgtmean, sd = fwgtsd) #Theoretical probability
sum(fdims$wgt < 75) / length(fdims$wgt) # Empirical probability
```
...
```{r}
library(ggplot2)
ggplot(fdims) + geom_histogram(mapping = aes(x= kne.di), fill = "cyan", color = "gray", binwidth = .5)
mean(fdims$kne.di)
median(fdims$kne.di)
```
